...
///
/// 定义在本文件而非 `shared/widgets/`：那里是**反向依赖方向**（widgets 依赖 theme），
/// 把判据放过去会让「谁依赖谁」反过来。改判据时只改这里——转出的是同一个声明，
/// 不会出现两份实现。
///
/// ⚠️ 这条注释曾经写成「与 `AppFocusableAction` 留在本文件同一原因（theme ↔ widgets
/// 循环依赖）」——那是**半截结论**：成环只在「只搬一个 widget」时成立（theme 还要
/// 用它）；12 个 widget 一起搬走后 theme 不再引用任何 widget，就不成环。
/// 现 widget 已全部迁到 `shared/widgets/`，本文件只剩令牌与主题。
///
