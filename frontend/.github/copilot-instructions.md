...
### Aesthetic Direction
- **护眼暖色主题（单强调色方案）+ 轻快愉悦基调**：低饱和暖白亮色背景 + 柔和植物绿单一强调色（Botanical Green `#5B8C5A`），并维护一套暖调暗色主题供夜间使用。色彩语义化，不发生硬编码打散主题。
- **同时支持亮色 + 暗色模式**：暗色不直接用纯黑，采用暖调深炭，保留植物绿强调色，确保晚间护眼。
...
2. **轻快但克制**：愉悦的圆润插画与弹性反馈服务于情绪目标，但配色仍单强调、低饱和、不炫技。
3. **单一强调色，克制配色**：颜色语义化（primary 主操作 / secondary AI 暖区 / tertiary 积极反馈 / error 温柔珊瑚），不引入新色相。
4. **亮暗双主题，护眼一致**：亮色护眼暖白、暗色暖调深炭，均走令牌，避免纯黑。
5. **一致性来自令牌而非临时值**：所有间距/圆角/字号走 `AppSpacing / AppRadius / textTheme`，禁止 `Colors.*` 硬编码。
6. **平板优先、护眼为先**：大圆角、暖白背景、低饱和，正文 ≥ 20sp。
