...
# 注：批量生成（自生成 + 自写库）的 `POST /batch-generate` 与单科兼容端点 `POST ""`
# 已在 ADR-0023 删除——它们构成「生成期并行写库」的分叉。现出题统一走
# `POST /ai/tasks/generate`（SSE 只读流）→ 前端确认 → `POST /tasks/from-generated`
# （唯一业务写库点）；整卷/单题重生成经下方 regenerate 路径复用同一共享生成核心。
