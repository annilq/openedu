...
- ⚠️ **图标按钮一律 `AppIconAction`**，别用 `CupertinoButton(padding: zero)`——它默认 `minSize` 44×44，是第三种高度权威、会撑高同行。命中区分两档：同行并排 = `AppIconAction`；输入框内部（`app_inputs.dart` 眼睛）= `AppControl.heightSmOf` 方形。
...
- pytest 前 `cd backend && mv .env .env.hidden`（跑完恢复），用 `.venv/bin/ruff`、`.venv/bin/pytest`；偶发 `Sensitive content approval timed out` → 重跑。`Resource<T>` 迁移未完成；`docs/adr/` 现 0001–0047。守卫 `tests/ai/`。
- 待真机：家长端密集表单控件 32→40 后纵向是否过长；轨态娃娃浮层从 53.5px 起飞、掠过轨道末边的观感。
