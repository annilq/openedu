...
> **架构变更先改文档与 ADR，再改代码。**（正常开发流程，见 `项目分析_架构规范与业务功能.md` §2.1）

任何影响分层、数据模型、API 契约、设计系统的改动，先更新对应文档与 ADR，再落地代码。文档与代码保持同步是该项目的硬要求（已多次整改验证）。

## ADR 机制

- 后端决策记录在仓库根 `娃娃学习App_ADR.md`（ADR-001..013）。
...
- 跨模块的「草稿 / 锁定 / 派发」「题库复用」「级联安全」等语义改动，参考 `实施文档_题库复用闭环_*.md`。
