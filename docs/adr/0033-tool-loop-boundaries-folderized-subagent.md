...
> **补充（非矛盾，2026-09-22 真机）**：「硬失败」还须覆盖**未查先答**（模型零工具调用却给出结论）。形态与上一档不同：正文里**没有**任何调用协议标记，就是一段像模像样的答复（本机 `qwen3:1.7b` / `ministral-3:3b` 在流式下直接编出「你有 3 个娃娃：小明 / 小红 / 小华」并附掌握度数字）。三档旧守卫（引擎不支持 / 无正文 / 正文即协议）一档都不命中，编造内容被当查询结论下发——**数据权限并未被绕过**（工具根本没执行，`resolve_children` 无从生效），但用户看到的是假的，且比越权更难发现。机制化闸门：`BaseSubAgent.requires_tool_data`（默认 False）声明「本 subagent 的结论必须来自工具数据」；`run_with_tools` 在整轮结束、正文下发前判定「从未成功执行过任何工具 → `ERROR(code="TOOL_UNSUPPORTED")` + `_UNVERIFIED_ANSWER_HINT`」，`native_fc_seen` 为真（前面确实查过）时放行收尾概括。由 `QuerySubAgent` 开启，并以契约测试 `test_query_subagent_requires_tool_data` 防回退。**已知代价**：该 subagent 不再能「不查就答」，连「你能查什么」这类元问题也会被拦——安全优先于体验，且文案给出可操作出路（重试 / 换支持原生工具调用的模型）。实测：同一模型同一 prompt，`stream=false` 正常返回 `tool_calls`、`stream=true` 直接编正文（`ministral-3:3b` 100% 复现；`qwen3:1.7b` 约 1/30 偶发），故「模型管理 → 测试连接」的探针不带 tools 属假绿灯（见 `app/features/model_management/probe.py`）。
...
> **补充（ADR-0042，非矛盾）**：双轨不变（`TOOL_RESULT` 仍存原始载荷、裁剪仍只在工具侧），但 `DATA` 载荷已从「拼好的展示串」（`{type, subject, stem}`）升级为**类型化卡片**：`kind` 作判别键进 `data.type`，`result` 只放结构化字段（`items` / `stats` / `total` / `text`）。原因是旧载荷把「标签」放在 `result.type`，与信封的 `data.type`（种类）同名异义，前端据此无法分派渲染器——「前端零改动」的代价被转移成了「前端无从改动」。
...
- **决策 11**：`query` 工具结果卡片投影双轨（呈现契约），裁剪只发生在工具侧（`backend/app/ai/subagents/query/render.py:9`）。卡片形状（kind + 结构化载荷）见 ADR-0042。
